<?php
	...
	if (isset($_POST["enviar"])) {
		if (!empty($_POST["artista-evento"]) && !empty($_POST["nome-evento"])) {
			...
			$inserir = $mysqli->query("UPDATE `eventos` SET 
					`artista_evento` = '$artista_evento',
					`data_evento` = '$data_evento',
					`local_evento` = '$local_evento',
					`nome_evento` = '$nome_evento',
					`detalhes_evento` = '$detalhes_evento'
					WHERE `id_eventos` = '$myid'");
	...
	$user = $mysqli->query("SELECT * FROM `eventos` WHERE `id_eventos` = '$myid'");
...
<html>
	<head>
		...
		<title>EDITAR Evento BACKEND - SITE</title>
	...
	<body class="admin-crm inicio-page">
		...
		<main class="pagina-geral"> 
			...
			<h6>Aqui pode gerir editar eventos</h6>
					...
					<div class="col-md-6 col-xs-12">
						<div class="campos">
							<div class="form-controls">
								<label>
									Artista:<br>
									<input type="text" name="artista-evento" value="<?php echo $printMenus->artista_evento; ?>">
								</label>
							</div>
							<div class="form-controls">
								<label>
									Data:<br>
									<input type="text" name="data-evento" value="<?php echo $printMenus->data_evento; ?>">
								</label>
							</div>
							<div class="form-controls">
								<label>
									Local:<br>
									<input type="text" name="local-evento" value="<?php echo $printMenus->local_evento; ?>">
								</label>
							</div>
					...
					<div class="col-md-6 col-xs-12">
						<div class="campos">
							...
							<div class="form-controls">
								<label>
									Nome:<br>
									<input type="text" name="nome-evento"  value="<?php echo $printMenus->nome_evento; ?>">
								</label>
							</div>
							...
							<div class="form-controls">
								<label>
									Detalhe:<br>
									<input type="text" name="detalhes-evento"  value="<?php echo $printMenus->detalhes_evento; ?>">
								</label>
							</div>
						</div>
					</div>
